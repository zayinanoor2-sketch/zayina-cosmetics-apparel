...
// Global state arrays synchronized via Firestore real-time listeners
let products = [];
...
// Default initial seed data if Firestore products collection is empty
...
function initFirebaseSync() {
 ...
 db.collection("resellerApplications").onSnapshot(snapshot => {
  resellerApps = [];
  snapshot.forEach(doc => {
   resellerApps.push({ firestoreId: doc.id, ...doc.data() });
  });
  renderAdmin();
 ...
 db.collection("reviews").onSnapshot(snapshot => {
  reviews = [];
  snapshot.forEach(doc => {
   reviews.push({ firestoreId: doc.id, ...doc.data() });
  });
  renderReviews();
 }, error => console.error(error));
}
